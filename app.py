...
def productos_clientes_from_nombre():
    ...
    if request.method == "POST":
        try:
            ...
            todos_los_productos_clientes = get_productos_clientes_from_nombre(
                nombre)
            productos_clientes = []
...
def single_venta_cab():
    ...
    try:
        ...
        tipo_entrega = stored_procedures.get_tipo_entrega_name_from_id(
            tipo_entrega_id)
...
def crear_orden_venta():
    ...
    if request.method == "POST":
        try:
            ...
            connection = getdb()
            stored_procedures = StoredProcedures(connection)
            nombre_cliente = stored_procedures.get_cliente_from_id(id_cliente)[
                0].razon_social
            ...
            for articulo in temp_list_of_articles:
                ...
                stored_procedures.add_to_articulos_reservados(
                    articulo[0], articulo[1], orden_venta_cab)
